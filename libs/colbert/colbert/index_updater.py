...
from colbert.utils.utils import batch, dotdict, flatten, lengths2offsets, print_message
...
class IndexUpdater:
    ...
    def persist_to_disk(self):
        ...
        print_message(f"#> The last chunk can hold {last_chunk_capacity} additional passages.")
        ...
        emb_last = emb_start + torch.sum(self.searcher.ranker.doclens[pid_start:]).item()

        # First populate the last chunk
        if last_chunk_capacity > 0:
            pid_end = min(pid_last, pid_start + last_chunk_capacity)
            emb_end = emb_start + torch.sum(self.searcher.ranker.doclens[pid_start:pid_end]).item()
    ...
    def _get_chunk_idx(self, pid):
        for i in range(self.metadata["num_chunks"]):
            ...
            if (
                ...
                and chunk_metadata["passage_offset"] + chunk_metadata["num_passages"] > pid
            ):
                return i
