...
# See: https://github.com/deepset-ai/haystack/blob/main/rest_api/controller/feedback.py
...
from schema import CreateLabelSerialized, FilterRequest
...
def export_feedback(
    ...
    for label in labels:
        ...
        if full_document_context:
            ...
            end_pos = min(
                offset_start_in_document + len(answer_text) + context_to_add,
                len(text) - 1,
            )
            additional_context_at_start = max(
                offset_start_in_document + len(answer_text) + context_to_add - len(text),
                0,
            )
